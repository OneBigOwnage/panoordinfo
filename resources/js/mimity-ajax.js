...
App.ajax = (config) => {
  ...
  if (url) {
    ...
    App.loadURL(url + window.location.search, document.querySelector(config.container))

    const currentModule = (url.split('?')[0]).split('/')[0]
    const currentLink = menu.querySelector('a[href="#' + currentModule + '"]')

    currentLink && App.updateMenu(currentLink)

    if (config.breadcrumb) {
      currentLink && App.updateBreadcrumb(currentLink)
    } else {
      document.getElementById('main-breadcrumb') && document.getElementById('main-breadcrumb').remove()
    }
